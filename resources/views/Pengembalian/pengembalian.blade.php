...
<body>
            ...
            <div class="content">
                ...
                <div class="search-box">
                    <form method="GET" action="{{ route('pengembalian.index') }}" style="margin-bottom: 20px;">
                        <input type="text" name="search" placeholder="Cari daftar peminjaman..."
                        ...
                        <form method="GET" action="{{ route('pengembalian.index') }}" style="margin-bottom: 20px;">
                            <select name="status" onchange="this.form.submit()"
                                style="padding: 8px 12px; background-color: #2563eb; color: white;margin-left: 6px; border: none; border-radius: 6px; font-size: 14px;">
                    ...
                    <tbody>
                        @forelse($pengembalian as $kembali)
                            <tr>
                                ...
                                <td>
                                    @if ($kembali->kondisi)
                                        <img src="{{ asset('storage/' . $kembali->kondisi) }}" alt="Kondisi Barang"
                                ...
                                <td>
                                    @if ($kembali->status == 'pending')
                                        <form action="{{ route('pengembalian.setuju', $kembali->id) }}" method="POST"
                                            style="display:inline">
                                            @csrf
                                            <button type="submit" class="btn-action btn-approve">Setujui</button>
                                        </form>
                                        <button type="button" class="btn-action btn-reject"
                                            onclick="bukaModal({{ $kembali->id }})">Tolak</button>
                                    @else
                                        <span>-</span>
                                    @endif
                                </td>
    ...
    <div id="modal-tolak"
        style="display:none; position: fixed; z-index: 1000; top: 0; left: 0; width: 100%; height: 100%; 
background: rgba(0,0,0,0.5); justify-content: center; align-items: center;">
        <div
            style="background:white; border-radius: 12px; max-width: 500px; width: 90%; padding: 20px; box-shadow: 0 0 20px rgba(0,0,0,0.3);">
            <h3>Alasan Tolak</h3>
            <form method="POST" id="form-tolak" action="">
                ...
                <div style="text-align: right;">
                    <button type="submit"
                        style="padding: 10px 16px; background-color: #ef4444; color: white; border: none; border-radius: 6px; cursor: pointer;">Kirim</button>
                    <button type="button" onclick="tutupModal()"
                        style="padding: 10px 16px; margin-left: 8px; background: #999; color: white; border: none; border-radius: 6px; cursor: pointer;">Batal</button>
                </div>
            </form>
        </div>
    </div>
